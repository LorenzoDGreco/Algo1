"""
Comentario importante sobre la entrega:

Como ya le hemos comentado a nuestro corrector, nosotros
hacemos pair programming utilizando Live Share de Vs Code
entre todo el grupo entonces por ello las funciones no
tienen nombre ya que hemos hecho todo entre todos
"""
# - Definición de funciones, importación de módulos e inicialización de constantes - #
# Python
import pathlib
import random
# Internas
...
from ahorcado_etapa_7 import ejecutar_interfaz_principal
...
def formar_diccionario_config(diccionario_config, contador_partidas):
    ...
    if (contador_partidas >= RESTO_DE_PARTIDAS):
        print("Considere que si desea cambiar la configuración del juego se reiniciarán los datos de la partida anterior.")
...
def generar_lista(diccionario_palabras, largo_palabra, diccionario_config, largo_palabra_max):
    ...
    >>> generar_lista({"cazar": 5, "casas": 1}, 5)
    ['casas', 'cazar']
    >>> generar_lista({"hola":1 ,"cazar": 5, "casas": 1}, 4)
    ['hola']
    """
...
def contar_puntos_por_palabra_completa(datos_jugador, diccionario_config):
    ...
    >>> contar_puntos_por_palabra_completa("ca?a", {"PUNTOS_ACIERTOS":10})
    10
    >>> contar_puntos_por_palabra_completa("c?s?", {"PUNTOS_ACIERTOS":10})
    20
    """
    palabra_censurada = datos_jugador["palabra_censurada"]
...
def gano_partida(datos_jugador):
    ...
    Post: Devuelve True si se ha ganado el juego, o False en el caso
          contrario.
    >>> gano_partida(0, "casa", "s", {"MAX_DESACIERTOS":8})
    True
    >>> gano_partida(1, "c???", "fin", {"MAX_DESACIERTOS":8})
    True
    >>> gano_partida(1, "c???", "0", {"MAX_DESACIERTOS":8})
    True
    >>> gano_partida(7, "c???", "v", {"MAX_DESACIERTOS":8})
    False
    >>> gano_partida(1, "c???", "a", {"MAX_DESACIERTOS":8})
    False
    """
    palabra_a_descubrir = datos_jugador["palabra_a_descubrir"]
    palabra_censurada =  datos_jugador["palabra_censurada"]
...
def almacenar_letras_ingresadas(datos_jugador):
    ...
    Post: Devuelve la lista que contiene las letras ingresadas con los cambios
          mencionados.
    >>> almacenar_letras_ingresadas("w",['q', 's', 'f', 'g'])
    ['q', 's', 'f', 'g', 'w']
    >>> almacenar_letras_ingresadas("q", ['q', 's', 'f', 'g'])
    ['q', 's', 'f', 'g']
    """
...
def contar_aciertos(datos_jugador):
    ...
    >>> contar_aciertos("w", "casa", 0)
    0
    >>> contar_aciertos("s", "casa", 0)
    Muy bien!!!
    1
    >>> contar_aciertos("a", "casa", 0)
    Muy bien!!!
    1
    """
    letra_a_verificar = datos_jugador["letra_a_verificar"]
...
def contar_desaciertos(datos_jugador):
    ...
    Post: Devuelve el acumulador de la cantidad de desaciertos (int) y los puntos
           restados (int)
    >>> contar_desaciertos("w", "casa", 1)
    Lo siento!!!
    2
    >>> contar_desaciertos("a", "casa", 0)
    0
    """
    letra_a_verificar = datos_jugador["letra_a_verificar"]
    palabra_a_descubrir = datos_jugador["palabra_a_descubrir"]
...
def almacenar_letras_erroneas(datos_jugador):
    ...
    Post: Devuelve la lista que contiene las letras erróneas con los cambios
          mencionados.
    >>> almacenar_letras_erroneas("w", "casa", ['q', 'r', 't', 'z'])
    ['q', 'r', 't', 'z', 'w']
    >>> almacenar_letras_erroneas("a", "casa", ['q', 'r', 't', 'z'])
    ['q', 'r', 't', 'z']
    """
    letra_a_verificar  = datos_jugador["letra_a_verificar"]
    palabra_a_descubrir = datos_jugador["palabra_a_descubrir"]
...
def desbloquear_letra(datos_jugador):
    ...
    >>> desbloquear_letra("s", "casa", "ca?a")
    'casa'
    >>> desbloquear_letra("a", "casa", "c?s?")
    'casa'
    >>> desbloquear_letra("a", "casa", "c?s?")
    'casa'
    >>> desbloquear_letra("w", "casa", "c?s?")
    'c?s?'
    """
    letra_a_verificar = datos_jugador["letra_a_verificar"]
    palabra_a_descubrir= datos_jugador["palabra_a_descubrir"]
...
def calcular_puntaje(datos_jugador, desaciertos_anterior, palabra_censurada_anterior, diccionario_config):
    ...
    >>> calcular_puntaje(0, "c?a?a", 0, "c???", {"PUNTOS_ACIERTOS":10})
    10
    >>> calcular_puntaje(1, "c???", 0, "c???", {"PUNTOS_DESACIERTOS":5})
    -5
    """
    desaciertos = datos_jugador["desaciertos"]
    palabra_censurada= datos_jugador["palabra_censurada"]
...
def mostrar_juego_general(datos_jugador_total, contador_partidas) :
    """
    Muestra un panorama general de cada jugador en las partidas que se llevan jugadas.
    Se muestran los puntos, aciertos, desaciertos y cantidad de partidas ganadas de
    cada jugador que hay en juego al terminar una partida.
    ...
    Post: imprime por pantalla los datos de cada jugador con respecto al total de partidas
        jugadas hasta el momento.
    """
    print("\n-------------- Panorama general ---------------------\n")
    print(f"Se jugaron hasta el momento {contador_partidas} partidas. \n")
    for jugador, informacion_juego in datos_jugador_total.items():
        PUNTOS = informacion_juego[0]
        DESACIERTOS = informacion_juego[1]
        ACIERTOS = informacion_juego[2]
        CANTIDAD_PARTIDAS_GANADAS = informacion_juego[3]
        print(f"El jugador {jugador} lleva acumulados {PUNTOS} puntos. \n" +
        f"Tuvo {ACIERTOS} aciertos, {DESACIERTOS} desaciertos, y gano {CANTIDAD_PARTIDAS_GANADAS} partidas.\n")
...
def ejecutar_ahorcado():
    ...
    listado_jugadores = ["Usuario1","Usuario2","Usuario3"]
    print("Hardcodeado: ", listado_jugadores)
    ...
    if (len(datos_jugador) != 0):
        ...
        while seguir_jugando(contador_partidas):
            ...
            while not any(list(diccionario_ganador.values())) and (turnos_jugadores_editable != []):
                ...
                print(datos_jugador[nombre_turno]["palabra_a_descubrir"]) #BORRAR ESTO

                mostrar_estado(datos_jugador[nombre_turno], nombre_turno)

                while not cambiar_turno:

                    datos_jugador[nombre_turno]["letra_a_verificar"] = ingresar_letra(datos_jugador[nombre_turno])
                    datos_jugador[nombre_turno]["letras_ingresadas"] = almacenar_letras_ingresadas(datos_jugador[nombre_turno])
                    datos_jugador[nombre_turno]["aciertos"] = contar_aciertos(datos_jugador[nombre_turno])
                    desaciertos_anterior = datos_jugador[nombre_turno]["desaciertos"]
                    datos_jugador[nombre_turno]["desaciertos"] = contar_desaciertos(datos_jugador[nombre_turno])
                    datos_jugador[nombre_turno]["letras_erroneas"] = almacenar_letras_erroneas(datos_jugador[nombre_turno])
                    palabra_censurada_anterior = datos_jugador[nombre_turno]["palabra_censurada"]
                    datos_jugador[nombre_turno]["palabra_censurada"] = desbloquear_letra(datos_jugador[nombre_turno])

                    datos_jugador[nombre_turno]["puntos"] += calcular_puntaje(datos_jugador[nombre_turno], desaciertos_anterior, palabra_censurada_anterior, diccionario_config)

                    if (datos_jugador[nombre_turno]["desaciertos"] > desaciertos_anterior) or (datos_jugador[nombre_turno]["letra_a_verificar"] in CADENAS_DE_FIN):
                        cambiar_turno = True
                    ...
                    print("\nTus puntos son: " + str(datos_jugador[nombre_turno]["puntos"]))
                    mostrar_resultado(datos_jugador[nombre_turno], diccionario_config)
                    if not diccionario_ganador[nombre_turno] or not cambiar_turno:
                        mostrar_estado(datos_jugador[nombre_turno], nombre_turno)
            ...
            if contador_partidas > 0:
                mostrar_juego_general(datos_jugador_total, contador_partidas)
